...
class ClientStatTableBody extends Component {
  ...
  getTerritory = async () => {
    let territoryData = await instance.get("/client/territory-list/", {
      params: {
        limit: 500,
      },
    });
  ...
  getCarLine = async () => {
    let carData = await instance.get("/client/car-list/");
    this.setState({
      carData: carData.data.results,
    });
  };
  getPriceTypeData = async () => {
    let priceTypeData = await instance.get("/client/price-type-list/?limit=1000");
    this.setState({
      priceTypeData: priceTypeData.data.results,
    });
  };
  getAgent = () => {
    const url = "/client/client-agents-partner-list/";
    instance.get(url).then((res) =>
      this.setState({ saleAgentData: res.data.results })
    );
  };
  getMarketType = async () => {
    const url = "/client/market-type-list/";
    instance.get(url).then((res) =>
      this.setState({ marketTypeData: res.data.results })
    );
  };
  onDelete = (e) => {
    const url = `/client/client-detail/`;
  ...
  onUpdate = (e) => {
    e.preventDefault();
    this.props.parentCallback(true);
    const url = `/client/client-detail/${this.state.id}/`;
    const {
      name,
      market_type,
      sale_agents,
