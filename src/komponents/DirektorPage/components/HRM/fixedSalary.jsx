...
class FixedSalary extends Component {
  ...
  setSalary = (event) => {
    const url = "/salary/fixed-salary-list/";
  ...
  getUsersData = async () => {
    const url = "/salary/fixed-salary-list/?limit=1000000";
  ...
  onEdit = (id) => {
    this.setState({ id });
    const url = "/salary/fixed-salary-detail/";
  ...
  onUpdate = (e) => {
    e.preventDefault();
    const url = `/salary/fixed-salary-detail/${this.state.id}/`;
    const { salary, role } = this.state;
    instance.put(url, {
      role,
      salary,
