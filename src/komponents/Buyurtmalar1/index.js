...
export default function Buyurtmalar() {
  ...
  const getSellOrder = () => {
    axios
      .get(`/order/sell-order-list/?offset=1000&limit=1000`)
      .then((response) => {
  ...
  const getSellOrderTwo = () => {
    axios
      .get(`/order/sell-order-list/?offset=0&limit=1000`)
      .then((response) => {
        let order = [];
    ...
    <div>
      <BuyurtmaHeader>
        ...
        <SubmitButton
          onClick={() => {
            console.log('salom');
            axios
              .post(`/order/sell-order-list/`, getTasdiqlashArray())
              .then((response) => {
      ...
      <div className="row" style={{ width: '100%', pading: '0', margin: '0' }}>
        <div className="col-md-3">
          <Klient
            clients={ordered}
            uzgartirSurov={(arg1, arg2) => {
              setSurovId(arg1);
              setSurovQuantitiy(getOneTasdiqlashMiqdori(arg1));
            }}
          />
        </div>
...
const KlinetZakazItem = (props) => {
  ...
  React.useEffect(() => {
    ...
    axios
      .get(`/order/sell-order-detail/${props.surovId}`)
      .then((response) => {
        console.log(response.data);
