...
const _BuyOrderPayment = () => {
  ...
  const fetchPaymentList = async () => {
    setLoading(true);
    try {
      const response = await axios.get("/order/buy-order-payment/", {
        params: { term: globalState.search },
      });
      setPaymentsList(response.data.payments);
      await axios.get("/order/buy-order-payment/", {
        params: {
  ...
  useEffect(() => {
    ...
    if (selector?.paymentList?.data) {
      console.log(selector?.paymentList?.data?.payments,'selector?.paymentList?.data')
      setPaymentsList(selector?.paymentList?.data?.payments);
    } else {
      console.log("else");
    }
  }, [selector]);
